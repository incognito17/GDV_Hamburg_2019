...
Um beide Choroplethenkarten interaktiv darstellen zu können wurde der Code einer skalierbaren Vektorgrafik (SVG) der Hamburger Stadtteile (Quelle: https://en.wikipedia.org/wiki/File:Hamburg_Subdivisions.svg) eingebaut. Der SVG-Code musste nach den Stadtteilen so editiert und gefiltert werden, dass Siedlungen innerhalb Hamburgs, die deutlich abgelegene Insel Neuwerk oder die Redundanz der Stadtteile aufgrund der zwei Choroplethenkarten die Visualisierungen nicht verfälschen. Die interaktiven Funktionalitäten, wie die Färbung der Gebiete, wurden mittels JavaScript programmiert.
